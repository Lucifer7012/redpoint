...
- 后续注意：
  - 当前已保存拒绝原因，但房主侧还没有专门的“对方拒绝原因”通知列表；后续可以基于 `roomInvites` 的 rejected 状态补一个反馈入口。
...
- 改动：
  ...
  - 结算后如果本地玩家是唯一赢家且本局有奖池，会弹出“领取奖励”确认。
  - 玩家点击“领取奖励”后才执行欢乐豆派奖，并刷新当前账号余额和排行榜数据。
