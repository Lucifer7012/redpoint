...
- 背景：上一轮只拦截了“等待中的好友房”切回单机，但只要房间已经开打、或者从对局返回大厅，仍可能出现大厅显示为“单机”而实际还有好友房未结束的状态错位。
- 改动：
  - 新增围绕“open friend room”的统一判断：只要当前账号还有 `waiting` 或 `playing` 状态的好友房，就视为仍在好友房流程中。
  - `单机` 页签在这种状态下会直接禁用；点击时会保留在“好友联机”，并提示先关闭或离开当前好友房。
  - `startGame()` 入口也补了同一层拦截，避免通过旧 UI 状态或其他路径绕过大厅页签限制。
  - `refreshSocialData()`、`renderAuthControls()` 和返回大厅路径现在都会自动把大厅模式同步回“好友联机”，覆盖“对局中点回到大厅”这种此前还会显示错模式的场景。
  - 缓存版本更新为 `20260610-open-room-solo-lock-sync`。
- 涉及文件：
  - `app.js`
  - `index.html`
  - `styles.css`
...
- 验证：
  ...
  - 最新提交为 `a3d3cc4 Lock solo mode while friend room is open`，本地与 `origin/main` 已同步到同一提交。
