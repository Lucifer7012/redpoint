...
- 改动：
  - `styles.css` 中只调整横屏对局公共牌区内部的 `table-public-slot`，加入安全内边距。
  - 同时补上 `box-sizing: border-box`，避免这层留白把公共牌区重新撑坏。
  - `index.html` 中把缓存版本更新到 `styles.css?v=20260707-public-card-clip-fix` 和 `app.js?v=20260707-public-card-clip-fix`。
- 涉及文件：
  - `styles.css`
  - `index.html`
  - `docs/CHANGELOG.md`
...
- 后续注意：
  - 如果还要继续修公共牌边距，优先动内部留白，不先扩大外框。

## 2026-07-06

### 右下三格文字放大但不越框

- 背景：用户确认边框和底边问题先这样，新的要求是把右下三张状态小框里的文字再放大一点，但不能重新超出框。
- 改动：
  - `styles.css` 中把 `selection-metrics` 的标签字和数值字各放大一档。
  - 同步略微收紧单卡内部的顶部内边距和文字间距，避免字号放大后又把框撑破。
  - `index.html` 中把缓存版本更新到 `styles.css?v=20260706-right-panel-metrics-text` 和 `app.js?v=20260706-right-panel-metrics-text`。
- 涉及文件：
  - `styles.css`
  - `index.html`
  - `docs/CHANGELOG.md`
...
- 验证：
  ...
  - 本地 `http://127.0.0.1:4173/` 返回 `200`。
- 后续注意：
  - 如果还要继续加大，优先先动数值行，再考虑框体高度。

### 右下三格继续缩短并露出底边

- 背景：用户最新截图继续指出，右下三张状态小框虽然已经改成直角，但最下面那条边还是不明显，视觉上像被裁掉；同时要求不要再改大右侧 `最近动作` 区块。
- 改动：
  - `styles.css` 中继续只处理横屏对局右下 `selection-metrics`：把整排上提，给底部留出一条可见空隙。
  - 同步缩小三格高度、格间距、内边距和文字尺寸，并补更清晰的边框与底边阴影，让底边更容易看出来。
  - `index.html` 中把缓存版本更新到 `styles.css?v=20260706-right-panel-bottom-align` 和 `app.js?v=20260706-right-panel-bottom-align`。
- 涉及文件：
  - `styles.css`
  - `index.html`
  - `docs/CHANGELOG.md`
...
- 验证：
  ...
  - 本地 `http://127.0.0.1:4173/` 返回 `200`。
- 后续注意：
  - 如果用户还觉得底边弱，优先继续压这三格自身，不先动 `最近动作` 的大小。
...
- 背景：用户最新反馈指出，大厅里的 `上一局结算` 还是旧网页感白卡，和当前牌桌的纸面 / 深青 / 暖金风格不统一；另外卡片内容太高，横屏时容易把整个结算区撑出画面。
- 改动：
  - `renderSetupHistory()` 不再输出长段红牌文本，而是改成更紧凑的排行卡结构：名次、玩家名、大分数、统计胶囊、`得分牌` 标签区。
  - `styles.css` 中重做了 `result-card`、`result-stats`、`result-score-chips` 等一整套样式，让它更贴近当前大厅和底部结算区的视觉语言。
  - 第一名卡片额外保留轻绿色层次，标题继续沿用现有书法标题字，不单独再引入另一套完全跳脱的 UI。
  - `history-panel`、`result-grid` 和 `result-score-chips` 都加了高度兜底和内部滚动；移动横屏下自动改单列并进一步收紧，优先防止超出画面。
  - 同步把缓存参数更新到 `styles.css?v=20260621-history-ui-refresh` 和 `app.js?v=20260621-history-ui-refresh`。
- 涉及文件：
  - `app.js`
  - `styles.css`
  - `index.html`
...
- 验证：
  - `node --check app.js` 通过。
  - 本地页面已确认加载 `20260621-history-ui-refresh` 缓存版本。
- 后续注意：
  - 如果后面还要继续压这块高度，优先动标签区滚动高度和卡片内边距，不要再回退到大段红牌文本。

### 回到大厅后默认收起上一局结算

- 背景：用户在最新截图里确认，打完一把点击“回到大厅”后，大厅中不希望自动出现整块 `上一局结算` 面板；这会把主入口往下挤，也不符合“回大厅先回到正常大厅态”的预期。
- 改动：
  - 在 `app.js` 里新增 `setupHistoryOpen` 状态，专门控制大厅历史结算是否展开。
...
- 验证：
  - `node --check app.js` 通过。
  - 人工复核大厅渲染条件与返回大厅逻辑，确认默认收起、手动点击才展开。
- 后续注意：
  - 如果之后还想继续压大厅高度，优先保留这种“按钮入口 + 按需展开”的方式，不要再回退成回大厅自动展开整块结算。

### 三张相同规则改正为 5 / 10 / J / Q / K

- 背景：用户继续澄清后确认，上一轮把“三张相同”限制成只对 `10 / J / Q / K` 生效仍然不对；目标规则应为：只有 `5 / 10 / J / Q / K` 才能在台面正好有 3 张同点牌时，用手里 1 张同点牌一次钓走那 3 张。
- 改动：
  - 在 `app.js` 中保留 `TEN_RANKS` 继续处理 `10 / J / Q / K` 的单张同点钓牌规则。
  - 新增 `TRIPLE_CAPTURE_RANKS`，统一控制 `5 / 10 / J / Q / K` 的“三张相同”资格。
  - `canUseTripleCapture()`、默认规则提示、手动选牌报错文案和规则状态文字全部改成按 `5 / 10 / J / Q / K` 生效。
  - `index.html` 的规则说明同步改正，并把 `app.js` 缓存参数更新到 `20260621-triple-capture-5`，降低线上继续拿到旧脚本的概率。
- 涉及文件：
  - `app.js`
  - `index.html`
  - `docs/CHANGELOG.md`
...
- 验证：
  - `node --check app.js` 通过。
  - 人工复核 `5` 的目标提示、三张相同资格判断和规则弹窗文案，确认已统一。
- 后续注意：
  - `5` 是这个规则里的特殊数字牌：既保留普通“5 钓 5”的凑十逻辑，也保留“三张 5”一次钓走的特殊逻辑；其他普通数字牌不要误套这条规则。

### 单机结算卡移除欢乐豆占位文案

- 背景：用户在移动横屏单机结算截图里确认，结算卡中的 `不输不赢` 和 `单机练习局不涉及欢乐豆` 会挤占 `得分牌` 的展示空间，但这两行在单机局本身没有信息价值。
- 改动：
  - 在 `app.js` 的结算渲染中按 `beansRound` 是否存在分流。
  - 纯单机练习局不再输出 `settlement-card__beans` 与 `settlement-card__detail`。
  - 欢乐豆/联机相关对局保持原有输赢和说明展示，不影响奖励领取逻辑。
- 涉及文件：
  - `app.js`
  - `docs/CHANGELOG.md`
  - `docs/PROJECT_STATUS.md`
...
- 验证：
  - `node --check app.js` 通过。
  - 人工复核单机结算卡结构，确认无欢乐豆局只保留名字、总分、身份与 `得分牌`。
- 后续注意：
  - 之后如果继续压缩移动横屏结算卡，优先保 `得分牌`，不要再把单机局的欢乐豆占位文案加回来。

### 三张相同规则只保留给 10 / J / Q / K

- 背景：这是 2026-06-21 当天较早一轮的中间修正记录，后续已被上面的“`5 / 10 / J / Q / K`”规则更正覆盖。保留这里仅为追溯当时错误收口的过程，不应再作为当前规则依据。
- 改动：
...
- 背景：用户继续截图指出，问题不只是三张小框的直角，而是右侧整列缺少像旁边手牌区那样完整的底边承托。
- 改动：
  - `styles.css` 中给横屏对局 `center-stage` 新增右侧列专用底板伪元素。
  - 让 `最近动作 + 三张指标卡` 共用一整块外层底板，补齐右侧列的完整底边。
  - `index.html` 同步抬高静态资源缓存参数到 `20260706-right-panel-base`。
- 涉及文件：
  - `styles.css`
  - `index.html`
- 验证：
...
- 背景：用户明确要求右下三张状态小框必须是 90 度直角，上一版小圆角不算长方形。
- 改动：
  - `styles.css` 中把横屏右下三张 `selection-metrics` 小框改为 `border-radius: 0`。
  - 同时补上整排拉伸和卡片满高，让三张卡的底边更整齐。
  - `index.html` 同步抬高静态资源缓存参数到 `20260706-metrics-hard-rect`。
- 涉及文件：
  - `styles.css`
  - `index.html`
- 验证：
...
- 背景：用户在浏览器控制台看到了大厅 `玩家人数` 自定义下拉的 `aria-hidden` 焦点告警；同时右下三张状态小框虽然已缩小，但边角仍然显得怪，希望改成长方形。
- 改动：
  - `app.js` 中关闭人数菜单时，若焦点仍在 `#player-count-menu` 内，先把焦点移回 `#player-count-trigger`。
  - `styles.css` 中把横屏右下三张 `selection-metrics` 小框继续收紧，并把圆角压低成更规整的长方形观感。
  - `index.html` 同步抬高静态资源缓存参数到 `20260706-focus-metrics-rect`。
- 涉及文件：
  - `app.js`
  - `styles.css`
  - `index.html`
...
- 背景：用户实机截图确认，结算里的 `得分牌` 标签花色全部显示成黑色；同时右下角三张状态小框略高，底边会顶出容器。
- 改动：
  - `app.js` 中大厅历史结算和对局结算两条渲染路径都补上红 / 黑花色类名。
  - `styles.css` 中补齐 `result-score-chip`、`settlement-card__score-card` 的红黑颜色。
  - 横屏对局只收紧右下角三张状态小框的高度、内边距、圆角和字号，不改 `最近动作` 区块尺寸。
  - `index.html` 同步抬高静态资源缓存参数到 `20260706-settlement-chip-metrics`。
- 涉及文件：
  - `app.js`
  - `styles.css`
  - `index.html`
- 验证：
  - `node --check app.js` 通过。
  - 本地静态页 `http://127.0.0.1:4173/` 返回 `200`。
