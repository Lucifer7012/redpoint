...
- 最近主要改动：手机横屏大厅已改为“单机 / 好友联机”切换；中间模式卡已固定为好友联机高度，切换时不再跳动；好友联机模式卡右侧新增竖向操作区，用来承载“关闭房间 / 离开房间 / 开始联机 / 返回对局”等按钮，避免按钮挤出房间卡边界；右侧好友列表 / 排行榜共用固定外框并滚动；房间邀请弹窗改为进入游戏大厅后才显示。

## 当前接力状态

- 上次做到哪里：已修正“登录成功但还没进入大厅就弹出房间邀请”的问题，并把好友联机模式卡里的房间操作按钮收纳到右侧竖向区域，避免超出边界。
- 下一步准备做什么：继续按手机横屏截图微调大厅布局，优先关注好友申请 / 房间邀请区域，以及好友联机模式卡的细节观感。
