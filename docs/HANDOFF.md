...
- 当前缓存版本：`styles.css?v=20260604-right-panel-unclipped`，`app.js?v=20260604-right-panel-unclipped`
- 最近主要改动：横屏小视口对局继续按真实截图做微调：上一轮右侧最近动作恢复后仍被手牌面板裁住，本轮把右侧信息层提到手牌面板之上，并把右侧栏放宽到 `244px - 278px`，中间手牌区相应让位；右侧应显示上方最近动作、下方三张指标卡，左侧牌堆视觉和张数仍保持略微下沉。此前已经完成的四区牌桌结构仍保留：中间大框只放公共牌区，底部整条只放手牌和 3 个操作按钮，左下小框放牌堆和状态提示，右下区域放最近动作与指标卡。

## 当前接力状态

- 上次做到哪里：横屏小视口对局已经完成结构性重排，并继续修正了右侧最近动作被手牌面板裁切的问题；现在右侧应是上方最近动作、下方三张指标卡，左侧牌堆内容比旧版略微下沉。底部手牌裁切此前已修掉，`915 x 412` 和 `844 x 390` 仍是主要验证尺寸。
