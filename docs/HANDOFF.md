...
- 当前缓存版本：`styles.css?v=20260610-open-room-solo-lock-sync`，`app.js?v=20260610-open-room-solo-lock-sync`
- 最近主要改动：在保持当前简洁斗地主式 CSS 扑克牌面与四人横屏公共牌扩宽逻辑不变的前提下，新增“好友房打开时锁住单机”规则。现在只要当前账号还有 `waiting` 或 `playing` 状态的好友房，大厅就会自动保持在“好友联机”，`单机` 页签会禁用；从对局返回大厅、或从云端恢复到现有好友房时，也会立刻同步回好友联机模式。当前牌面仍只显示点数/字母和红黑花色图案，不显示“钓牌 / 计分”文字，不加载 `assets/cards/daoist/` 或 `<img class="card-face">`。2 人局 10 张手牌放不下时继续横向滚动。

## 当前接力状态

- 上次做到哪里：当前版本仍是非图片牌方案，CSS 牌面保持白底红黑牌；`13 张` 公共牌会展开成 `7 + 6`，`15 张` 会展开成 `8 + 7`，不再继续浪费左右留白，也不会因为过早进入 3 行而把第一行裁掉。随后又补了一轮大厅模式保护：如果当前还有好友房未结束，就不能切回单机，也不能通过旧 UI 状态绕过规则开始单机；从对局返回大厅时也会自动同步回“好友联机”。
- 下一步准备做什么：如果继续调牌桌，优先基于最新 `public-area-columns` 和 `card-symbol-spacing` 两轮结果做细节微调；如果继续调大厅逻辑，优先围绕好友房/单机切换的状态一致性继续补验证。不要再从道风图片牌版本继续改。若用户要换另一套牌，先确认是 CSS 样式牌、图片牌，还是只做临时预览。
- 当前先别动什么：不要重新接入 `assets/cards/daoist/`；不要随意放松“好友房打开时锁住单机”的规则，除非本轮任务明确要求恢复允许并行单机。
