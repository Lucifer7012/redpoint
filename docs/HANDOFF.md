...
- 当前缓存版本：`styles.css?v=20260604-landscape-action-visible`，`app.js?v=20260604-landscape-action-visible`
- 最近主要改动：横屏小视口对局继续按真实截图修正右侧最近动作。已确认 Chrome 响应式预览和真实触屏横屏可能命中不同媒体条件，所以本轮在 CSS 末尾新增 `body.is-game-view` 横屏兜底规则，统一普通横屏和触屏横屏下的底部三栏牌桌；右侧最近动作强制显示为左文字、右动作牌，并保留下方三张指标卡。本机 Chrome `844 x 390` 预览图 `artifacts/layout-check/chrome-after-landscape-fallback-844x390.png` 已确认右侧最近动作可见。

## 当前接力状态

- 上次做到哪里：横屏小视口对局已经完成结构性重排，并补了普通横屏 / 触屏横屏共用的兜底规则；最新截图验证里右侧最近动作和动作牌已经可见。底部手牌裁切此前已修掉，`915 x 412` 和 `844 x 390` 仍是主要验证尺寸。
