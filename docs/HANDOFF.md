...
- 当前缓存版本：`styles.css?v=20260604-right-panel-content-locked`，`app.js?v=20260604-right-panel-content-locked`
- 最近主要改动：横屏小视口对局继续按真实截图修正右侧最近动作。本轮在右侧栏绝对定位的基础上，把 `action-stage` 内部也锁成左文字、右动作牌两列，并显式恢复文字区和牌区的可见性，修正“外框出现但内容不见”的问题。右侧栏仍保持 `244px - 278px`，中间手牌区继续让位；左侧牌堆视觉和张数保持略微下沉。

## 当前接力状态

- 上次做到哪里：横屏小视口对局已经完成结构性重排，并把右侧最近动作锁定到右侧栏；最新一轮又把右侧框内部锁成左文字、右动作牌，避免外框有但内容不见。底部手牌裁切此前已修掉，`915 x 412` 和 `844 x 390` 仍是主要验证尺寸。
