...
- 实际修法：继续只动 `selection-metrics` 单卡内部，把标签字和主数值各放大一档；同时略微收紧顶部内边距和文本间距，避免放大后重新溢出。
- 当前缓存版本：`styles.css?v=20260706-right-panel-metrics-text`，`app.js?v=20260706-right-panel-metrics-text`。
- 本轮校验：`node --check app.js` 通过；本地静态页 `http://127.0.0.1:4173/` 返回 `200`。
- 如果用户还觉得不够大，下一步优先先加粗或继续小幅放大 `strong` 行，不要先去改三格整体高度。

## 2026-07-06 Right Bottom Edge Follow-up

- 最新补充反馈：用户确认右下三张小框的真正问题是“最下面的边看不到”，并明确表示可以接受把这三格做得更小一些，但不要继续动右侧 `最近动作` 的整体尺寸。
- 实际修法：继续沿用上一轮右侧整列底板；这轮只把 `selection-metrics` 整排从贴底状态抬起，减小高度、间距、内边距和字号，并补强单卡底边描线，让三格下方稳定露出一条可见底部留白。
- 当前缓存版本：`styles.css?v=20260706-right-panel-bottom-align`，`app.js?v=20260706-right-panel-bottom-align`。
- 本轮校验：`node --check app.js` 通过；本地静态页 `http://127.0.0.1:4173/` 返回 `200`。
- 如果用户还觉得底边不够明显，下一步优先继续微调这三格自身的高度 / bottom 间距 / 边框对比度，不要先去改 `最近动作` 卡片高度。

## 2026-07-06 Right Column Base Follow-up

- 最新补充反馈：用户指出“看不到底边”，真正想要的是右侧这一整列像旁边手牌区一样有完整底部承托，而不只是把三个小框改成直角。
- 实际修法：在横屏对局 `center-stage` 上新增右侧列专用底板伪元素，覆盖 `最近动作 + 三张指标卡` 整个右槽位，提供完整外层边框和底边。
- 当前缓存版本：`styles.css?v=20260706-right-panel-base`，`app.js?v=20260706-right-panel-base`。
- 本轮校验：`node --check app.js` 通过。
- 下一步如果用户还觉得不像旁边，就继续微调这块底板的圆角、边框亮度和顶部间距，不要只盯着三个小框本身。
