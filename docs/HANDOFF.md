...
- 之后用户继续核对玩法，指出当前实现把“三张相同”错误放宽到了所有点数。现在已确认并收口为目标规则：普通数字牌只按凑十，只有 `10 / J / Q / K` 才能触发“三张相同”。
- 这次修正不是单点补丁，而是一起改了 AI、玩家手动选择判定、可点击高亮、规则提示和规则说明文案；如果公司电脑继续跟进玩法问题，先默认这些入口已经重新统一，不要只盯某一个函数。
- 因此像“最后桌面剩 4 张 4”这种残局，之后不能再用“三张相同也许能吃掉其中 3 张”去解释；按当前目标规则，它们只能靠 `6` 去凑十。
...
- 当前缓存版本：`styles.css?v=20260615-lobby-custom-select`，`app.js?v=20260615-lobby-custom-select`
- 最近主要改动：大厅 `玩家人数` 已从系统原生下拉切到轻量自定义菜单，解决 Windows/Chrome 弹层过大和右侧空白灰块的问题，同时保留隐藏原生 `#player-count` 作为真实数据源；普通出牌钓牌和补枪两条链路的右侧“最近动作”仍统一为只显示目标牌；手牌区按剩余手牌数量连续重算 gap / overlap 的逻辑仍保留。此前补上的真实对局页覆盖修复、轻量道风视觉美化、“单机局从大厅返回/关闭”以及“单机未关闭时锁好友联机”逻辑也仍保留。

## 当前接力状态

- 上次做到哪里：已经把大厅人数控件从原生 `select` 切成自定义下拉，并补齐了关闭 / 切模式 / 离开大厅时的收口逻辑；普通钓牌和补枪 target-only 动作展示、手牌区连续铺开的逻辑也都还在。当前没有改玩法和整体布局框架。
- 下一步准备做什么：优先让用户在真实设备上确认“大厅人数下拉框视觉是否顺眼”、“普通钓牌 / 补枪最近动作都只显示目标牌”和“10 -> 9 -> 8 张手牌逐步铺开”这三条最新交互；如果还想继续抠细节，再微调大厅自定义下拉的阴影、展开位置和动作区小牌尺度。
