...
- 最近主要改动：手机横屏大厅已改为“单机 / 好友联机”切换；中间模式卡已固定为好友联机高度，切换时不再跳动；右侧好友列表 / 排行榜共用固定外框并滚动；房间邀请弹窗改为进入游戏大厅后才显示。

## 当前接力状态

- 上次做到哪里：已修正“登录成功但还没进入大厅就弹出房间邀请”的问题，邀请弹窗现在只在大厅态显示。
- 下一步准备做什么：继续按手机横屏截图微调大厅布局，优先关注好友申请 / 房间邀请区域和右侧好友 / 排行榜区域。
