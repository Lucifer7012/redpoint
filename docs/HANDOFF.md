...
- 当前缓存版本：`styles.css?v=20260604-right-action-restored`，`app.js?v=20260604-right-action-restored`
- 最近主要改动：横屏小视口对局继续按真实截图做微调：左侧牌堆视觉和张数整体下移一点，右侧信息区恢复“最近动作”上半块，下方继续放当前出牌、已选台面牌、判定目标三张指标卡，避免只剩 3 个框、最近动作消失。此前已经完成的四区牌桌结构仍保留：中间大框只放公共牌区，底部整条只放手牌和 3 个操作按钮，左下小框放牌堆和状态提示，右下区域放最近动作与指标卡；公共牌、动作展示牌和手牌保持接近正常扑克牌比例，`915 x 412` 和 `844 x 390` 这类安卓横屏视口里，底部按钮和整排手牌应完整留在一屏内。本轮缓存版本提升到 `20260604-right-action-restored`。

## 当前接力状态

- 上次做到哪里：横屏小视口对局已经完成结构性重排，并在最新一轮修掉右侧最近动作消失的问题；现在右侧应是上方最近动作、下方三张指标卡，左侧牌堆内容也比上一版略微下沉。底部手牌裁切此前已修掉，`915 x 412` 和 `844 x 390` 仍是主要验证尺寸。
