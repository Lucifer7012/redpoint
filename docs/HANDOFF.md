...
- 最近主要改动：对局牌面已从旧文字/CSS 扑克牌切换为道风图片牌面，并按实机反馈改成单行大牌。52 张 256 x 384 WebP 位于 `assets/cards/daoist/`，显示比例固定为 2:3；触屏横屏公共牌约 `83-90px` 宽，手牌约 `82-87px` 宽，右侧最近动作牌约 `57px` 宽。公共牌强制单行横排，超过 12 张时继续保持单行；2 人 10 张手牌超过 7 张时启用 `is-scroll-hand` 横向滚动。4 人模式对手座位已改为左 / 中 / 右，2 号和 4 号玩家挪到两侧空位。同步脚本已补上 `assets` 和 `artifacts/layout-check`，后续新资源和截图会随 GitHub 同步。

## 当前接力状态

- 上次做到哪里：横屏小视口对局已经完成结构性重排、真实对局页 JS 固定右侧最近动作、底部整行高度压缩、公共牌大外框收窄、4 人骰子阶段专用布局，以及道风图片扑克牌接入、继续放大、公共牌/手牌单行排列和 4 人左右座位下移。已用真实 `index.html` 生成 4 人 `915 x 412`、4 人 `844 x 390`、2 人 `915 x 412` 新牌面截图：`artifacts/layout-check/real-index-large-single-cards-4p-915x412.png`、`artifacts/layout-check/real-index-large-single-cards-4p-844x390.png`、`artifacts/layout-check/real-index-large-single-cards-2p-915x412.png`；`large-single-cards-check.json` 记录坏图数为 0，公共牌行数为 1，手牌行数为 1。
