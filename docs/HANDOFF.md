...
- 当前缓存版本：`styles.css?v=20260604-right-panel-locked`，`app.js?v=20260604-right-panel-locked`
- 最近主要改动：横屏小视口对局继续按真实截图修正右侧最近动作位置。本轮不再只调层级，而是把 `action-stage` 从底部网格排版里拿出来，改为绝对定位锁在右侧栏，固定放在三张指标卡上方，避免它继续跑到中间手牌面板上。右侧栏仍保持 `244px - 278px`，中间手牌区继续让位；左侧牌堆视觉和张数保持略微下沉。此前四区牌桌结构仍保留。

## 当前接力状态

- 上次做到哪里：横屏小视口对局已经完成结构性重排，并把右侧最近动作锁定到右侧栏，不再参与中间手牌网格排版；现在右侧应是上方最近动作、下方三张指标卡，左侧牌堆内容比旧版略微下沉。底部手牌裁切此前已修掉，`915 x 412` 和 `844 x 390` 仍是主要验证尺寸。
