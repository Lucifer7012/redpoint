...
- 当前缓存版本：`styles.css?v=20260614-hand-action-target-only`，`app.js?v=20260614-hand-action-target-only`
- 最近主要改动：普通出牌钓牌和补枪两条链路的右侧“最近动作”都已经统一成只显示目标牌；大厅 `玩家人数` 下拉框也已收紧到更贴近当前大厅 UI 的尺度；手牌区按剩余手牌数量连续重算 gap / overlap 的逻辑仍保留。此前补上的真实对局页覆盖修复、轻量道风视觉美化、“单机局从大厅返回/关闭”以及“单机未关闭时锁好友联机”逻辑仍保留。

## 当前接力状态

- 上次做到哪里：已经把普通钓牌和补枪两条动作链路都统一成只显示目标牌，并补了独立预览页；大厅人数下拉框收紧和手牌区连续铺开的逻辑也仍在。当前没有改玩法和整体布局框架。
- 下一步准备做什么：优先让用户在真实设备上确认“普通钓牌 / 补枪最近动作都只显示目标牌”、“大厅人数下拉框不再过大”和“10 -> 9 -> 8 张手牌逐步铺开”这三条最新交互；如果还想继续抠细节，再微调最近动作小牌和大厅原生下拉的展示边界。
