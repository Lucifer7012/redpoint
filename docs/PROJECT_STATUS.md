...
- 玩法规则已补做一次关键收口：`三张相同` 现在只允许 `10 / J / Q / K` 触发，普通数字牌统一只按凑十处理；此前“所有点数都能三张相同”的实现已被纠正。
- 这轮不是只改一处判断，而是把 AI 选牌、玩家手动判定、可点击高亮、规则提示和规则弹窗文案一起同步到同一套规则。
- 当前如果再看到残局里留着多张 `4`、`5`、`6` 之类普通数字牌，不应再用“三张相同”去解释，它们现在只能靠凑十对应牌处理。
...
- 当前缓存版本仍是 `styles.css?v=20260618-settlement-layout-fix`，`app.js?v=20260618-settlement-layout-fix`；本轮只改 CSS 细节，没有再改缓存参数。
- 本轮已完成校验：`node --check app.js`；本地移动横屏模拟确认左侧提示条与牌堆框不再重叠，结算卡 `得分牌` 一行也可完整显示。
