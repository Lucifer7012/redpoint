...
- 账号 ID 详情改为点击“当前账号已绑定唯一 ID”里的 ID 后弹出小浮层，联机门票和当前模式战绩默认不再占用大厅下方空间；战绩资料未同步或暂无正式战绩时也会兜底显示当前账号信息。
- 对局结束后可查看结算榜单，再选择再来一局或回到大厅。
- 对局结束后桌面保留每名玩家的得分卡，原手牌区改为显示本局欢乐豆输赢明细，房主、赢家和输家都会显示净赢/输结果；赢家需要在结算卡里手动选择直接领取或看广告翻倍领取，结算状态会隐藏手牌区顶部说明和身份框，让结果卡上移以适配 100% 缩放。
