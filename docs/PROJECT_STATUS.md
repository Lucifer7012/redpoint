...
- 登录页、创建 ID 页和大厅页已拆分：未登录只显示登录/注册入口；新账号注册或登录后如果还没有游戏 ID，会先进入创建 ID 步骤；绑定 ID 后才进入大厅选择单机、好友房或查看排行。
...
- 已开始移动端 H5 适配：手机宽度下登录页已增加横屏游戏入口布局，未登录时横屏为左侧标题/玩法入口、右侧登录/注册面板；创建 ID/大厅/社交区暂沿用现有移动布局，后续可继续按斗地主大厅思路重做；手机对局页参考手机斗地主牌桌层级，竖屏保持固定一屏压缩牌桌，横屏新增独立的一屏游戏桌布局，顶部是紧凑功能按钮，中间是公共牌主区域，右侧显示最近动作和牌堆，底部是手牌与操作按钮，弹窗内容过长时在内部滚动。
