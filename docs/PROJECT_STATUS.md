...
最新状态补充（2026-06-10）：
- 只要当前账号还有 `waiting` 或 `playing` 状态的好友房，大厅现在就会强制保持在“好友联机”，不再允许切回单机。
- `单机` 页签会在有打开的好友房时直接禁用；即使通过旧 UI 状态或别的入口触发 `startGame()`，底层也会再次拦截并提示先关闭或离开当前好友房。
- 社交数据刷新、云端恢复现有好友房、以及“对局中点回到大厅”这几条路径，现在都会自动把大厅模式同步回“好友联机”，避免标签显示和实际可用状态不一致。
