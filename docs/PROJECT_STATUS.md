...
- 已开始移动端 H5 适配：手机横屏登录页已改为类似手游入口的上下布局，上方是游戏名和玩法简介，下方是邮箱 / Google 登录方式；点击邮箱登录后弹出居中表单，登录成功后显示“开始游戏”和右上角 ID / 退出；创建 ID/大厅/社交区暂沿用现有移动布局，后续可继续按斗地主大厅思路重做；手机对局页参考手机斗地主牌桌层级，竖屏保持固定一屏压缩牌桌，横屏新增独立的一屏游戏桌布局，顶部是紧凑功能按钮，中间是公共牌主区域，右侧显示最近动作和牌堆，底部是手牌与操作按钮，弹窗内容过长时在内部滚动。
