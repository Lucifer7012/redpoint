...
- 已继续跟进真实对局页里的手牌区问题：上一轮自适应算法本身没有问题，但正式 `is-game-view` 横屏样式把重叠规则覆盖回去了，导致 `2 人 10 张` 在真实页里仍不重叠、最后两张被裁掉。
- 现在真实对局页已补上更具体的 `.hand-grid.is-overlapped` / `.hand-grid.is-condensed` 规则，确保正式页和预览逻辑一致：能完整展示就不重叠，贴边能放下就只贴边，贴边还不够才自动重叠。
- 这套逻辑仍作用于共享的本地手牌区，所以单机 / 好友联机共用；2 / 3 / 4 人模式都会按当前手牌数量和容器宽度动态切换。
- 为了让压缩态更稳定，手牌进入压缩或重叠后会取消奇偶高低差；进入重叠态时再按牌序重新设定覆盖层级，被选中的手牌继续保持抬起优先级。
...
- 本轮已完成校验：`node --check app.js`；使用本机 Chrome headless 重新导出 `915 x 412` 预览图，确认 `2 人 10 张` 已完整重叠展示，`3 人 7 张` 仍只贴边不重叠。
