...
- 更新 `index.html` 中 `styles.css` / `app.js` 缓存版本为 `20260602-lobby-status-wrap`。
- 同步补充项目协作约定：后续 Codex 每次改完按公司电脑格式补齐记录文档，并自动检查、提交、推送到 GitHub。
验证：
- `node --check app.js` 通过。
- 本地 `http://127.0.0.1:4173/` 页面可加载新版 `styles.css?v=20260602-lobby-status-wrap` 和 `app.js?v=20260602-lobby-status-wrap`，控制台无错误。
