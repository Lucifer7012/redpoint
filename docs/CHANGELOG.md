...
### 右下三格文字回调放大
- 根据最新截图继续微调横屏对局右下三张状态小框，这次不再改框体大小，只把框内标题字和数值字适度放大，让信息更容易看清。
- 为了避免放大后重新顶出边界，`selection-metrics` 单卡内部同步略微收紧了顶部内边距和文本间距，保持文字仍然完整落在框内。
- 同步抬高静态资源缓存参数到 `styles.css?v=20260706-right-panel-metrics-text` 与 `app.js?v=20260706-right-panel-metrics-text`。
Verification:
...
### 右下三格上提并露出完整底边
- 继续收口横屏对局右下角三张状态小框的底边问题：这次不再扩大右侧区域，也不动 `最近动作` 卡片本身，只把最底下一排三格整体缩短并上提，让底边明确露出来。
- `styles.css` 中将 `selection-metrics` 从贴底状态改为保留底部留白，同时下调高度、间距、内边距和字号，并补一层更清晰的外描边 / 底边阴影，让三个直角框在深绿色桌布上更容易看出完整边界。
- 继续保留 90 度直角矩形，不把圆角加回去；这轮重点是“看得到底边”和“不要再像被截断”。
- 同步抬高静态资源缓存参数到 `styles.css?v=20260706-right-panel-bottom-align` 与 `app.js?v=20260706-right-panel-bottom-align`。
Verification:
...
- 用户继续截图确认，问题不只是三个指标卡的圆角，而是右侧这一整列缺少像旁边手牌区那样完整的底部承托，导致“看不到底边”。
- 这轮在横屏对局的 `center-stage` 里为右侧列新增整块底板背景和边框，让 `最近动作 + 三张指标卡` 共用一块完整的底部承托，视觉上更接近旁边的手牌区。
- 保留三张指标卡的 90 度直角，不把圆角加回去；这轮主要补的是整列外层边界。
- 同步抬高静态资源缓存参数到 `styles.css?v=20260706-right-panel-base` 与 `app.js?v=20260706-right-panel-base`。
...
- 右下角三张状态小框继续收口，但这次不再保留偏胶囊的圆角比例，而是改成更规整的矮长方形：进一步收紧高度、间距、字号和内边距，并把圆角降到更接近矩形的尺度。
- 同步抬高静态资源缓存参数到 `styles.css?v=20260706-focus-metrics-rect` 与 `app.js?v=20260706-focus-metrics-rect`，避免浏览器继续命中上一版圆角样式。
...
- 根据用户最新反馈，大厅里的 `上一局结算` 视觉上太像旧网页卡片，不够贴当前“米白纸感 + 深青 + 暖金”的牌桌风格；同时单张卡内容太高，在横屏小视口里容易把整个结算区撑到超出画面。
- 这轮把 `renderSetupHistory()` 的内容改成更紧凑的排行卡结构：名次、玩家名和总分在上半区集中展示，中间改成两枚信息胶囊，底部把长段红牌文字改成 `得分牌` 小标签列表。
- 样式上没有再沿用旧的“白底大段文本框”，而是重做成更接近当前大厅 / 牌桌的纸面卡片：更浅的纸感底、柔和内高光、第一名带轻绿色放射层次、标题继续沿用现有书法标题字。
- 同时给 `history-panel` 和 `result-score-chips` 增加了高度兜底和内部滚动，移动横屏下历史结算会自动收成单列，并限制在更稳的可视高度内，避免整块面板继续冲出屏幕。
- 补同步缓存参数：`styles.css` 与 `app.js` 已一起更新到 `20260621-history-ui-refresh`，避免线上仍命中旧样式或旧结构。

Verification:

- `node --check app.js` passed.
- 本地页面已确认加载 `styles.css?v=20260621-history-ui-refresh` 与 `app.js?v=20260621-history-ui-refresh`。
