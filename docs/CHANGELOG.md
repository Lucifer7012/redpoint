...
- 规则、欢乐豆、房间邀请弹窗限制最大高度，手机上内容过长时在弹窗内滚动。
- 更新 `index.html` 中 `styles.css` / `app.js` 版本参数为 `20260525-mobile-pass1`，避免浏览器继续使用旧缓存。

验证：

- `node --check app.js` 通过。
- 本地服务 `http://127.0.0.1:4173/index.html?verify=mobile-pass1` 返回 200。
- 内置浏览器确认页面加载 `styles.css?v=20260525-mobile-pass1` 和 `app.js?v=20260525-mobile-pass1`，控制台无错误。
